...
import cog from "./img/settings.svg";
import check from "./img/check-circle.svg";
import './App.css';

import {kanji} from "./Kanjis/strings";
import kanjijson from "./Kanjis/kanjidata.json";
  ...
  renderKanjiBlock(block, i){
    return(
      <div className="kanjiblock" key={i}>
        {block.map((kanji, index) => this.renderKanji(kanji, (i*220)+index,20))}
      </div>
    );
  }
    ...
    return (
      <div className="App">
        <div className="box" >
          ...
          <h1 className="logo"> Kanji App</h1>
        </div>
      </div>
    );
  }
